...
void write_img(double *img, int w, int h, int dst_x, int dst_y)
{
	int y, x;
	...
	for(y=h-1; y>=0; y--) {
		for(x=w-1; x>=0; x--) {
			int v = pal_max * (img[y*w+x] - vmin) / (vmax - vmin);
			if(v < 0) v = 0;
...
void run(hid_device *handle, int w, int h)
{
	...
	double img[60][80] = { 0 };
	double img_re[60][80] = { 0 };
	double img_im[60][80] = { 0 };
	...
	for(;;) {
		...
		if(y == h-1) {
			...
			if(n == n_cycle) {
				...
				double img_abs[60][80] = { 0 };
				double img_phase[60][80] = { 0 };
				for(int y=0; y<h; y++) {
					for(int x=0; x<w; x++) {
						img_abs[y][x] = hypot(img_re[y][x], img_im[y][x]);
						img_phase[y][x] = atan2(img_im[y][x], img_re[y][x]);
					}
				}

				if(fact_c > 0) {
					printf("bli\n");
					write_img(&img_abs[0][0], w, h, win_w * 0.0, win_h * 0.5);
					write_img(&img_phase[0][0], w, h, win_w * 0.5, win_h * 0.5);
				}

...
unsigned xvformat(void (*outchar)(char), void *arg, const char *fmt, va_list args);
...
static unsigned outBuffer(void (*myoutchar)(char), const char *buffer, int len, unsigned toupper)
{
	unsigned count = 0;
	int i;
	char c;
...
static unsigned outChars(void (*myoutchar)(char), char ch, int len)
{
	unsigned count = 0;
...
unsigned xvformat(void (*outchar)(char), void *arg, const char *fmt,
		  va_list args)
{
	...
	while(*fmt) {
		...
		case ST_TYPE:
			...
			param.count += outBuffer(
			    outchar, param.prefix, param.prefixlen,
			    param.flags & FLAG_UPPER);
			if(!(param.flags & FLAG_LEFT))
				param.count +=
				    outChars(outchar, param.pad,
					     param.width);
			param.count += outBuffer(
			    outchar, param.out, param.length,
			    param.flags & FLAG_UPPER);
			if(param.flags & FLAG_LEFT)
				param.count +=
				    outChars(outchar, param.pad,
...
{
	void outchar(char c) { tx((uint8_t)(c)); }

	(void)xvformat(outchar, NULL, fmt, va);
}


static rv printd_tx(uint8_t c) { return uart_tx(c); }
...
{
	fhexdump(uart_tx, addr, len, offset);
}
